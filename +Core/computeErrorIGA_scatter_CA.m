function [matErrL2,matErrH1] = computeErrorIGA_scatter_CA(compErrorStruct)
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % EXTRACT REFERENCE SOLUTION
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    [pp,e,t,ffSol,stiffMat,massMat] = my_FFimportfilemesh('ffMesh.msh','ffSolution.sol','A.txt','M.txt');
    ...
    [nn,mm] = size(compErrorStruct); 
    matErrL2 = zeros(nn,mm);
    matErrH1 = zeros(nn,mm);
    
    for aa = 1:nn
        for bb = 1:mm
            
            plotStruct = compErrorStruct{aa,bb};

            size_mb     = plotStruct.dimModalBasis;
            a_ril       = plotStruct.liftCoeffA;
            b_ril       = plotStruct.liftCoeffB;
            cutx        = plotStruct.domainLimit_inX;
            hx          = plotStruct.stepMeshX;
            u           = plotStruct.u;
            bc_up       = plotStruct.label_upBoundDomain;
            bc_down     = plotStruct.label_downBoundDomain;
            Coeff_forma = plotStruct.coefficientForm;
            caso        = plotStruct.simulationCase;
            p           = plotStruct.degreePolySplineBasis;
            k           = plotStruct.continuityParameter;
            space       = plotStruct.space;
            geometry    = plotStruct.refDomain1D;
            map         = plotStruct.geometricInfo.map;                   
            ...
            higaSol = griddata(X,Y,solMat,pp(1,:),pp(2,:));
            ...
            matErrL2(aa,bb) = errL2;
            matErrH1(aa,bb) = errH1;
