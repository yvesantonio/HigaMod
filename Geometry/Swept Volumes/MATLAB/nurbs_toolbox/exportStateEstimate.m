function [dataFileSurf,errH1,errL2] = exportStateEstimate (size_mb,a_ril,b_ril,cutx,hx,...
    ...
    stateEstimate   = zeros(2*nnx,y_elements);
    stateEstimate_x = zeros(2*nnx,y_elements);
    stateEstimate_y = zeros(2*nnx,y_elements);
    ...
    for h = 1:2*nnx
        ...
        for k = 1:M
            ...
            for imb = 1:size_mb
                ...
                stateEstimate(h,k)   = stateEstimate(h,k)   + u(h+(imb-1)*2*nnx)*coeffModalBase(k,imb);

                % COMPUTATION OF THE DERIVATIVE OF THE APPROXIMATED
                % SOLUTION VECTOR FIELD EVALUATED IN THE POINTS OF THE
                % DOMAIN MESH

                stateEstimate_x(h,k) = stateEstimate_x(h,k) + ux(h+(imb-1)*2*nnx)*coeffModalBase(k,imb);
                stateEstimate_y(h,k) = stateEstimate_y(h,k) + u(h+(imb-1)*2*nnx)*coeffModalBaseDer(k,imb);

            end

        stateEstimate(h,k) = stateEstimate(h,k)+ a_ril(1) * evalNodesY(k) + b_ril(1);
    ...
    % dataFileSurf = 0;
        ...
        dataFileSurf = sprintf('dataEstimate.%d.vtu',iteration - 1);
        TRI = delaunay(X,Y);
        vtktrisurf(TRI,X,Y,stateEstimate','ADRConcentration',dataFileSurf);
...
%     obsvMatrix = zeros(2*nnx,M);
%     
%     for ii = 1:2*nnx
%         
%         obsvMatrix(ii,:) = obsvVec((ii - 1) * M + 1:ii * M);
%         
%     end
    
    %maxOBM = max(max(obsvMatrix))
    %minOBM = min(min(obsvMatrix))
    
%     err0 = abs(sol_ex'   - obsvMatrix  );
    
    err1 = abs(sol_ex'   - stateEstimate  );
    err2 = abs(sol_exdx' - stateEstimate_x);
    err3 = abs(sol_exdy' - stateEstimate_y);
    
    % COMPUTATION OF THE L2 NORM OF THE ERROR

%     errL20 = sqrt(sum(sum(err0.^2))/sum(sum(sol_ex.^2)));
