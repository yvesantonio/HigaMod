...
% % DEBUG
% disp(geometry.map)
% disp(geometry.map_der)
% disp(geometry.map_der2)
% 
...
% 
% XXr = reshape(XX,[n^2,1])';
% YYr = reshape(YY,[n^2,1])';
% 
% [newC] = geometry.map([XXr;YYr]);
% [J] = geometry.map_der([XXr;YYr]);
% 
% J = geometry.map_der([1.0;0.5]);
% Hes = geometry.map_der2([0.3;0.7]);
% 
% disp(J)
% 
...
% 
% % fp(ones(1,5))
% % disp(mapx(ones(1,1),v,map))
% % disp(mapy(ones(1,1),v,map))
% % disp(mapOut([.1,.2,.3],v,map,2))
% 
% mX = reshape(newC(1,:),[n,n])';
% mY = reshape(newC(2,:),[n,n])';
% 
% % disp(J)
% 
% [mX,mY] = meshgrid([.1,.2,.3],v);
% 
% YY = mapOut([.1,.2,.3],v,map,2);
% XX = mapOut([.1,.2,.3],v,map,1);
% 
% % disp(XX)
% % disp(YY)
% % 
