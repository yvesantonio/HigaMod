function [errL2,errH1] = computeErrorIGA_scatter_3D(size_mb,a_ril,b_ril,cutx,...
    ...
    y_elements = length(evalNodesTrans);
    ...
    type = geoInfo.Type;
    [Xref,Yref,Zref] = meshgrid(evalNodesTrans,evalNodesTrans,evalNodesX);
...
%     xslice = 0.0;                               % define the cross sections to view
%     yslice = 0.0;
%     zslice = [0.0 0.5 1.0];
% 
%     slice(Xref, Yref, Zref, solMat, xslice, yslice, zslice)    % display the slices
%     xlim([0 1])
%     ylim([0 1])
%     zlim([0 1])
%     view(135,25)
...
%     [X,Y,Z] = mapOut3DHiMod(evalNodesX,evalNodesTrans,evalNodesTrans,geoInfo,type);
    ...
    % form the axes
%     x = 0:0.2:10;   % first dimension indipendent variable
%     y = 0:0.2:10;   % second dimension indipendent variable
%     z = 0:0.2:10;   % third dimension indipendent variable
