...
for i=1:numel(surf_point)
    m=numel(surf_point{i}(:,1));
    if m>max
        max=m;
    end
end
%% uniforma tutti i punti rispetto al massimo
for i=1:numel(surf_point)
    i
    [aa nur]=nrb_approx(surf_point{i}(:,1),surf_point{i}(:,2),surf_point{i}(:,3),15,3,1.e-10,20,100);
    eval=nrbeval(nur,linspace(0,1,max));
    surf_point{i}=eval';
end
...
vett2=surf_point{i}(45,:)-coordtg(i,:);
