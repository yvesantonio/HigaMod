...
fsimeshgeneratorbis = 'FSImeshgeneratorbis'

class FSImeshgeneratorbis(pypes.pypeScript):
    ...
    def ComputeSmartRadius(self,surfaceIn,centerlines,cellentityidsarrayname,radiusarrayname,distancetocenterlinearrayname,smartdistancearrayname):
        ...
        distanceToCenterlinesFilter.SetInput(surfaceIn)
    ...
    def Execute(self): 
        ...
        print "interface mesh extraction"
        threshold = vtk.vtkThreshold()
        threshold.SetInput(surfaceRemeshed)
        ...
        boundaryExtractor.SetInput(interfaceSurface)
        ...
        for i in range (boundaries.GetNumberOfCells()):
            ...
            mass.SetInput(meshToSurface.Surface)
            mass.Update()
...
#-------------------------------------------------------------------------------------------------------------
        ...
        if self.Fluid == 1:
            ...
            if bound == "y":
                ...
                appendFilterBL.AddInput(interfaceSurface)
                appendFilterBL.AddInput(boundaryLayer.Mesh)
                appendFilterBL.Update()            
                #tetra the result
                tetrahedralizeBL = vtk.vtkDataSetTriangleFilter()
                tetrahedralizeBL.SetInput(appendFilterBL.GetOutput())
                tetrahedralizeBL.Update()
                #this is only for region growing!
                appendFilterBL2 = vtkvmtk.vtkvmtkAppendFilter() 
                appendFilterBL2.AddInput(innerSurface)
                appendFilterBL2.AddInput(tetrahedralizeBL.GetOutput())
                ...
                threshold = vtk.vtkThreshold()
                threshold.SetInput(remeshing.Surface)
                ...
                BLboundaryExtractor.SetInput(innerSurface)
                ...
                sizingFunction = vtkvmtk.vtkvmtkPolyDataSizingFunction()
                sizingFunction.SetInput(normalsFilter.Surface)
                sizingFunction.SetSizingFunctionArrayName(SizingFunctionArrayName)
                sizingFunction.SetScaleFactor(VolumeElementScaleFactor)
                sizingFunction.Update()           
                ...
                #final tetrahedralization (results = volumetric mesh with all interfaces)
                tetrahedralizeFluid = vtk.vtkDataSetTriangleFilter()
                tetrahedralizeFluid.SetInput(tetgen.Mesh)
                tetrahedralizeFluid.Update()
...
##-------------------------------------------------------------------------------------------
        ...
        if self.Solid:
            ...
            while region_delineation == 1:
                ...
                mapper.SetInput(Meshsurf.Surface)
                ...
                select.SetInput(Meshsurf.Surface)
            ...
            while region_delineation == 1:
                ...
                mapper = vtk.vtkPolyDataMapper() 
                mapper.SetInput(surfaceExternal)
                mapper.ScalarVisibilityOn()
                mapper.SetScalarModeToUsePointData()
                mapper.SetColorModeToMapScalars()
                mapper.UseLookupTableScalarRangeOn() 
                ...
                interactor.Start() 
                select = vtk.vtkSelectPolyData()
                select.SetInput(surfaceExternal)
                select.SetLoop(polycontour.GetPoints())
                region = raw_input("Do you want the Smallest (1) or The Biggest (2) region? (Default Smallest)")
                if region == "2":
                    select.SetSelectionModeToLargestRegion()
            ...
            appendFilterSolid.AddInput(Extrusion.Mesh)
            appendFilterSolid.AddInput(InterfaceMesh)
            appendFilterSolid.AddInput(surfaceExternal)
            ...
            thre = vtk.vtkThreshold()
            thre.SetInput(solidTetra)
            thre.ThresholdBetween(10-1,10+1)
            thre.SetInputArrayToProcess(0, 0, 0, 1, "celltypes")
            thre.Update()       
            #Append surfaceTotale and the volumetric mesh
            appendFilterSolid = vtkvmtk.vtkvmtkAppendFilter() 
            appendFilterSolid.AddInput(surfaceTotale)
            appendFilterSolid.AddInput(thre.GetOutput())
            appendFilterSolid.Update()
    ...
    ##-----------------------------------------------------------------------
            ...
            boundaryExtractorExt.SetInput(meshToSurfaceExternal.Surface)
