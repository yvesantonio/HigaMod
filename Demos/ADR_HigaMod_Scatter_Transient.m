...
%     close all
    ...
    numbControlPoints = numbElements * continuityParameter + degreeSplineBasis + 1 -continuityParameter;
    ...
    case {7,8} 
        ...
        bt = @(x,y,t) 1000 + 0*x + 0*y + 0*t;
        br = @(x,y,t) 0 + 0*x + 0*y + 0*t;
        b  = @(x,y,t) sqrt(bt(x,y).^2 + br(x,y).^2) + 0*t;
        
        % Modulating function to give the flow a parabolic profile with
        % respect to the direction of flow
        
        fm = @(x,y,t) (sqrt(x.^2 + y.^2) - R1) .* (sqrt(x.^2 + y.^2) - R2) * (-4/((R1 + R2)^2)) + 0*t;
        
        % Equations for the horizontal and vertical components of the
        % convective field
        
        poiFlow1 = @(x,y,t) fm(x,y) .* b(x,y) .* cos( -pi/2 + atan2(y,x)) + 0*t;
        poiFlow2 = @(x,y,t) fm(x,y) .* b(x,y) .* sin( -pi/2 + atan2(y,x)) + 0*t;
