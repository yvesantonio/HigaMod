...
% Reaction differential problem in 2D using the HIGAMod solution.
    ...
    vectorModes=[7,9,11,13];
    vectorStepHorMesh=[0.5,0.025];
    %vectorModes=[3,7,13];
    ...
    h         = stepHorMesh;
    nel       = numbElements;
    ins       = sort(reshape((h:h:max_x-h)'*ones(1,continuityParameter),1,continuityParameter*(nel-1)));
...
%                         - 4*x.^3.*sin(2*pi*(y+0.5));  
    ...
    tic;
    [u2,a,b,L2_2,H1_2] = solverIGARTS(obj_solverIGA);
    toc;
