...
% Reaction differential problem in 2D using the HIGAMod solution.
    ...
    numbModes       = 5;
    nd              = length(numbModes); 
    stepHorMesh     = (max_x-min_x)*0.01*ones(size(numbModes));
    ...
    case 2
        ...
        domainProfile  = @(x) cof*x.^2;
        domainProfileDer = @(x) 2*cof*x;  
    ...
    case 3
        ...
        mu    = @(x,y) (  1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  0 + 1*x + 0*y ); % Horizontal Advection
        beta2 = @(x,y) (  0 + 0*x + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
    ...
    tic;
    [u2,a,b,L2_2,H1_2] = solverFEM(obj_solverIGA);
    toc;
