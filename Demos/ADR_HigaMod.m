...
% Reaction differential problem in 2D using the HIGAMod solution.
    ...
    case 6
        a     = @(x) sin(pi*x/5);                   % Equation of the Inferior Bound of the Channel (Considered As Central)
        ...
        domainProfile  = @(x) 0.5 * sin(pi * x/5);
        domainProfileDer = @(x) 0.1 * pi * cos(pi * x/5);
    ...
    case 3
     ...
     case 7
        ...
        domainProfile  = @(x) (1 - sqrt(1 - x.^2));
        domainProfileDer = @(x) (0.5 * (2 * x)./sqrt(1 - x.^2));
    ...
    case 2
        ...
        true_sol  = @(x,y) sin(x * pi/maxHor) .* sin(y * pi/maxVer);
        true_solx = @(x,y) cos(x * pi/maxHor) .* sin(y * pi/maxVer) * pi/maxHor;
        true_soly = @(x,y) sin(x * pi/maxHor) .* cos(y * pi/maxVer) * pi/maxVer; 

    case 3

        true_sol  = @(x,y) (x.^2).*(y.^2);
        true_solx = @(x,y) 2 .* x .* y;
        true_soly = @(x,y) 2 .* x .* y; 
    ...
    case 5 % Homogeneous Neumann B.C. in the Vertical direction
        
        true_sol  = @(x,y) cos(pi*y/maxVer) * sin(pi*x/(2*maxHor));
        true_solx = @(x,y) cos(pi*y/maxVer) * cos(pi*x/(2*maxHor)) * (pi/(2*maxHor));
        true_soly = @(x,y) sin(pi*y/maxVer) * sin(pi*x/(2*maxHor)) * (-pi/(maxVer));
        
    case {6,7,8} % Homogeneous Neumann B.C. in the Vertical direction
        
        true_sol  = @(x,y) cos(pi*y/maxVer) * sin(pi*x/(2*maxHor));
        true_solx = @(x,y) cos(pi*y/maxVer) * cos(pi*x/(2*maxHor)) * (pi/(2*maxHor));
        true_soly = @(x,y) sin(pi*y/maxVer) * sin(pi*x/(2*maxHor)) * (-pi/(maxVer));
        
    case {10} % Homogeneous Neumann B.C. in the Vertical direction
...
%         force = @(x,y) ((x-0.2).^2+(y - 0.5).^2<0.01);
        
        force = @(x,y) -(4*x.^3-(maxHor^3)).*sin(2*pi*((y/maxVer)+0.5))...
                       +(0.2*x.^5-0.5*(maxHor^3)*x.^2)*4*pi^2.*sin(2*pi*((y/maxVer)+0.5));              

    case 2

        dato_dir = @(y) 0;
        force = @(x,y)  mu(x,y) .* (...
                        sin(x * pi/maxHor) .* sin(y * pi/maxVer) * (pi/maxHor)^2 + ...
                        sin(x * pi/maxHor) .* sin(y * pi/maxVer) * (pi/maxVer)^2);

    case 3

        dato_dir = @(y) 0;
        force = @(x,y)  -2 * (x + y + 2*x.*y);
                    
    case 4

        dato_dir = @(y) 0;
        force = @(x,y) ((x-0.2).^2+(y - 0.25).^2<0.01);  
        
    case {5,6,7}
        
        dato_dir = @(y) 0;
        force = @(x,y) mu(x,y) .* (pi^2/(4*maxHor^2)) .* cos(pi*y/maxVer) .* sin(pi*x/(2*maxHor)) + ...
                       mu(x,y) .* (pi^2/(maxVer^2)) .* cos(pi*y/maxVer) .* sin(pi*x/(2*maxHor)) + ...
                       beta1(x,y) .* cos(pi*y/maxVer) .* cos(pi*x/(2*maxHor)) .* (pi/(2*maxHor)) + ...
                       beta2(x,y) .* sin(pi*y/maxVer) .* sin(pi*x/(2*maxHor)) .* (-pi/(maxVer)) + ...
                       sigma(x,y) .* cos(pi*y/maxVer) .* sin(pi*x/(2*maxHor));
    
    case 8

        dato_dir = @(y) 0;
        force = @(x,y) 1;%10 * ((x-1.5).^2+(y-.5).^2<0.25); 
    ...
    %% Analyse the problem with Freefem++
    
    % Domain information
    
    Domain = struct('up',maxVer,'down',minVer,'left',minHor,'right',maxHor);
    
    % Freefem++ mesh information
    
    numbHorNodes = 100;
    numbVerNodes = 100; % Pay attention! For now it is a fixed value!
    ...
    % Perform nalysis using Freefem++
    
    % [L2,H1] = analysisFreeFem(Domain, Mesh,Coeff,force);
    
    % Analysis results
    
%     disp('Maximum L2 Norm Error with Freefem++');
%     disp(max(max(L2)));
%     
%     disp('Maximum H1 Norm Error with Freefem++');
%     disp(max(max(H1)));
    
    disp(['L2-Norm Error : ',num2str(L2_2)]);
    disp(['H1-Norm Error : ',num2str(H1_2)]);