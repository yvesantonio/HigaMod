...
% Reaction differential problem in 2D using the HIGAMod solution.
    ...
    %Two loops,one over modes and one over stepHorMesh
    vectorModes=[3,5,7,9,11,13,15];
    vectorStepHorMesh=[0.05,0.025,0.0125,0.00625];
    %vectorModes=[3,7,13];
    %vectorStepHorMesh=[0.025];
    Delta=[0.01];
    for mm=1:length(vectorModes)
    for kk=1:length(vectorStepHorMesh)
    for dd=1:length(Delta)
    
    domainLimit_inX      = [min_x,max_x];  
    domainLimit_inY      = [min_y,max_y];
    
    numbModes       = vectorModes(mm);
    nd              = length(numbModes); 
    stepHorMesh     = (max_x-min_x)*vectorStepHorMesh(kk)*ones(size(numbModes));
    ...
    case 2
        ...
        mu    = @(x,y) (  1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  100 + 0*x + 0*y ); % Horizontal Advection
        beta2 = @(x,y) (  0 + 0*x + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction

    case 3

        mu    = @(x,y) (  0.1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  0 + 0*x + 10*sin(8*y) ); % Horizontal Advection
        beta2 = @(x,y) (  0 + 10*sin(8*x) + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
        
    case 4

        mu    = @(x,y) (  0.1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  10 + 0*x + 0*y ); % Horizontal Advection
        beta2 = @(x,y) (  10 + 0*x + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
        
     case 5

        mu    = @(x,y) (  0.1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  10*sqrt(6)/2 + 0*x + 0*y ); % Horizontal Advection
        beta2 = @(x,y) (  10*sqrt(2)/2 + 0*x + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
        
    case 6

        mu    = @(x,y) (  0.1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  10*sqrt(2)*cos(15/180*pi) + 0*x + 0*y ); % Horizontal Advection
        beta2 = @(x,y) (  10*sqrt(2)*sin(15/180*pi) + 0*x + 0*y ); % Vertical Advection
        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
...
%                         - 3*x.^2.*sin(2*pi*(y+0.5));     
    ...
    case 3
        ...
        dato_dir = @(y) 0;
        len = L;
         force =@(x,y) 10*(((x-0.2).^2 + 4.*(y-0.).^2) < 0.01);
...
%                         - 4*x.^3.*sin(2*pi*(y+0.5));  
    case 4
        ...
        dato_dir = @(y) 0;
        len = L;
         force =@(x,y) 10*(  ((x-0.2).^2 + (y-0.).^2) < 0.01  );
%         force = @(x,y)  (1/5*x.^5-len^4*x).*4*pi^2.*sin(2*pi*(y+0.5))+...
%                         (x.^4-len^4).*sin(2*pi*(y+0.5))...
%                         - 4*x.^3.*sin(2*pi*(y+0.5));  
...
%                         - 4*x.^3.*sin(2*pi*(y+0.5));  
    ...
    %Parameter for stabilizaztion: delta
    obj_solverIGA.delta = delta;

    tic;
    [u2,a,b,L2_2,H1_2] = solverIGASTR(obj_solverIGA);
    toc;
