...
% Reaction differential problem in 2D using the HIGAMod solution.
    ...
    numbModes       = 20;
    nd              = length(numbModes); 
    stepHorMesh     = (max_x-min_x)*0.002.*ones(size(numbModes));
    ...
    dato_dir_up   = 0.0;
    dato_dir_down = 0.0;
    chi           = @(x,y) (  1 + 0*x + 0*y ); % Robin coefficient
    mu            = 1;
    cest          = 1;
    ...
    case 1
        ...
        L     = @(x) (max_y - min_y) + 0*x; % Thickness of the Channel
        D1    = 0;
        D2    = 1;                          %/(max_y - min_y);
    ...
    case 2
        ...
        mu    = @(x,y) (  .1 + 0*x + 0*y ); % Difusion
        beta1 = @(x,y) (  7 + 0*x + 0*y ); % Horizontal Advection
    ...
    case 5
        ...
        true_sol  = @(x,y) (1/2*x.^2-L*x).*sin(2*pi*(y+0.5));
        true_solx = @(x,y) (x-L).*sin(2*pi*(y+0.5));
        true_soly = @(x,y) (1/2*x.^2-L*x)*2*pi.*cos(2*pi*(y+0.5));
...
%         force = @(x,y) ((x-0.2).^2+(y - 0.5).^2<0.01);
    ...
    case 2
        ...
        dato_dir = @(y) 0;
        force = @(x,y) 10*(( ((x-0.5).^2 + 4*(y-0.25).^2) < 0.01) + ( ((x-0.5).^2 + 4*(y-0.75).^2) < 0.01));
    ...
    case 5
        ...
        dato_dir = @(y) 0;
        len = (max_y - min_y);
        force = @(x,y) (1/2*x.^2-len*x).*4*pi^2.*sin(2*pi*(y+0.5))+(x-len).*sin(2*pi*(y+0.5))...
            - sin(2*pi*(y+0.5));
...
%     disp(times')
    ...
    Domain = struct('left',min_x,'right',max_x,'up',max_y,'down',min_y);
    Mesh = struct('numbHorNodes',1000,'numbVerNodes',1000);
    Coeff = struct('chi',chi,'mu',mu,'beta1',beta1,'beta2',beta2,'sigma',sigma);
    
    createFreeFem(Domain, Mesh,Coeff,force);