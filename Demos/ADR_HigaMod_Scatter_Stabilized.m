...
%% CHOICE OF ALL SIMULATION PARAMETERS TO BE TESTED
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Step used to dicretize the supporting fiber
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Note: To perform convergence analysis the typical choice for the vector
% of steps to discretize the supporting mesh is [ 0.2 0.1 0.05 0.025 0.0125].

stepIGA     = [0.05];


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Choice for the definition of the Pechlet number to be used
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Note: The options available in this moment for the stabilization method
% to be used are: 
%
% (01) 'UpwindSTR' : Upwind Artifitial Diffusion in Configuration
%                    Stabilize-then-reduce
% (02) 'UpwindRTS' : Upwind artifitial diffusion in configuration
%                    reduce-then-stabilize
% (03) 'StreamlineSTR' : Streamline Artificial Diffusion in Configuration
%                        Stabilize-then-reduce
% (04) 'StreamlineRTS' : Streamline Artificial Diffusion in Configuration
%                        Reduce-then-stabilize
% (05) 'SupgSTR' : Streamline Upwind Petrov-Galerkin in Configuration
%                  Stabilize-then-reduce
% (06) 'SupgRTS' : Streamline Upwind Petrov-Galerkin in Configuration
%                  Reduce-then-stabilize
% (07) 'GlsSTR' : Galerkin Least-Square in Configuration
%                 Stabilize-then-reduce
% (08) 'GlsRTS' : Galerkin Least-Square in Configuration
%                 Reduce-then-stabilize
% (09) 'DwSTR' : Douglas-Wang in Configuration Stabilize-then-reduce
% (10) 'DwRTS' : Douglas-Wang in Configuration Reduce-then-stabilize
% (11) 'None' : Solution without any stabilization method

stabMethod  = ["UpwindSTR"];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Choice of stabilization delta
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

stabDelta   = [1];

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Definition of error matrices
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

ErrorL2Matrix = zeros(length(modalCoeff),length(stepIGA),length(PeConfig),length(stabMethod),length(stabDelta));
ErrorH1Matrix = zeros(length(modalCoeff),length(stepIGA),length(PeConfig),length(stabMethod),length(stabDelta));
CutMatrix = cell(length(modalCoeff),length(stepIGA),length(PeConfig),length(stabMethod),length(stabDelta));

%%%%%%%%%%%%%%%%
% Exported Plots
%%%%%%%%%%%%%%%%

% Note: To select a given plot, it suffies to set its correspondent value
% in the selection vector as '1'. Otherwise, every plot with parameter set
% to '0' will not be plotted. The plots current available in this
% simulation are:
%
% (1) Contour plot of the Stabilized HigaMod solution;
% (2) Contour plot of the Freefem++ reference solution;
% (3) Contour plot of the Error of the Stabilized HigaMod solution;
% (4) Plot of quarter cut line of HigaMod and reference solution;
% (5) Convergence Analysis
% (6) Confrontation Error with Freefem++

N = 6;
selectedPlots = zeros(1,N);
selectedPlots(1) = 0;
selectedPlots(2) = 0;
selectedPlots(3) = 0;
selectedPlots(4) = 1;
...
refSolStructComp.flag = 0;
refSolStructComp.P = [];
refSolStructComp.E = [];
refSolStructComp.T = [];
refSolStructComp.SOL = [];
refSolStructComp.A = [];
refSolStructComp.M = [];

for aa = 1:length(modalCoeff)
    for bb = 1:length(stepIGA)
        for cc = 1:length(PeConfig)
            for dd = 1:length(stabMethod)
                for ee = 1:length(stabDelta)
                    %% Simulation case   
                    ...
                    numbModes       = modalCoeff(aa);
                    nd              = length(numbModes); 
                    stepHorMesh     = (maxHor-minHor)*stepIGA(bb)*ones(size(numbModes));
                    ...
                    case {1,2,3,4,5,6}
                        ...
                        beta1 = @(x,y) (  1e2 + 0*x + 0*y ); % Horizontal Advection
                        beta2 = @(x,y) (  0 + 0*x + 0*y ); % Vertical Advection
                        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction
                        
                        normL2mu = sqrt(Wid * Len);
                        normL2beta = 1e2 * sqrt(Wid * Len);

                    case {7,8} 

                        %% Poiseuille Flow Ring
                    ...
                    case {9,10}
                        ...
                        %% Sinusoidal Flow Ring

                        % Parameters of the convective field
                        ...
                        bt = @(x,y) 10 + 0*x + 0*y;
                        br = @(x,y) 100 * sin(Mr * atan2(y,x));
                        b  = @(x,y) sqrt(bt(x,y).^2 + br(x,y).^2);

                        % Equations for the horizontal and vertical components of the
                        % convective field

                        sinFlow1 = @(x,y) b(x,y) .* cos( -pi/2 + atan2(y,x));
                        sinFlow2 = @(x,y) b(x,y) .* sin( -pi/2 + atan2(y,x));

                        % Definition of the bilinear coefficients

                        mu    = @(x,y) (  1 + 0*x + 0*y ); % Difusion
                        beta1 = @(x,y) sinFlow1(x,y); % Horizontal Advection
                        beta2 = @(x,y) sinFlow2(x,y); % Vertical Advection
                        sigma = @(x,y) (  0 + 0*x + 0*y ); % Reaction

                    end

                    Coeff_forma = struct('mu',mu,'beta1',beta1,'beta2',beta2, ...
                    'sigma',sigma,'coeffrobin',chi,'normL2mu',normL2mu,'normL2beta',normL2beta);
...
% 
%                         dato_dir = @(y) 0;
%                         force = @(x,y) K1 * (((x - Cx1).^2 + (y - Cy1).^2) < r1^2) + ...
%                                        K2 * (((x - Cx2).^2 + (y - Cy2).^2) < r2^2) + ...
%                                        K3 * (((x - Cx3).^2 + (y - Cy3).^2) < r3^2);
                    ...
                    obj_solverIGA.stabMethod = stabMethod(dd);
                    obj_solverIGA.stabDelta = stabDelta(ee);
                    obj_solverIGA.PeConfig = PeConfig(cc);
                    obj_solverIGA.selectedPlots = selectedPlots;
                    obj_solverIGA.refSolStruct = refSolStructComp;

                    tic;
                    [u2,a,b,L2,H1,CutCoord,refSolStructComp] = solverIGAScatterStabilized(obj_solverIGA);
                    toc;
                    ...
                    ErrorL2Matrix(aa,bb,cc,dd,ee) = L2;
                    ErrorH1Matrix(aa,bb,cc,dd,ee) = H1;
                    CutMatrix{aa,bb,cc,dd,ee} = CutCoord;
                end
            end
        end
    end
...
%% PLOT THE CUTS ON THE SOLUTION

if(selectedPlots(4) == 1)
    ...
    % Note: It is usefull to solve the problem with a sufficiently refined
    % mesh in order to obtain the reference profile for the centerline
    % (second quarter cut line) and the first and third quarter cut lines.
    ...
    numbModes       = 15;
    nd              = length(numbModes); 
    stepHorMesh     = (maxHor-minHor)*0.001*ones(size(numbModes));
    numbElements    = round((maxHor-minHor)/stepHorMesh);
    ...
    case {2}
        ...
        normL2beta  = beta1(1,1) * sqrt(Len * Wid);
        normL2mu    = mu(1,1) * sqrt(Len * Wid);
...
%                                        K3 * (((x - Cx3).^2 + (y - Cy3).^2) < r3^2);
    ...
    tic;
    [u2,a,b,L2,H1,CutCoordRef,refSolStructComp] = solverIGAScatterStabilized(obj_solverIGA);
    toc;
    
    %% Plot cuts for each method

    for jj = 1:length(PeConfig)
        for ii = 1:length(stabMethod)

            N = 103;
            
            %% Plot centerline of the domain

            figg1 = figure;
            for kk = 1:length(stabDelta)
                M = CutMatrix{:,:,jj,ii,kk};
                plot(linspace(0,Len,N),M(1,:),'-o','LineWidth',1,'MarkerSize',2);
                set(gcf, 'Color', 'w');
                xlim auto
                ylim auto
                grid on
            ...
            xlabel('x [m]');
            ylabel('u(x,y,\delta)');
            plot(linspace(0,Len,N),CutCoordRef(1,:),'--k','LineWidth',1,'MarkerSize',2);
            LegendTitles = cell(1,length(stabDelta));
            for kk = 1:length(stabDelta)
                LegendTitles{kk} = ['\delta = ' num2str(stabDelta(kk))];
            end
            LegendTitles{kk + 1} = ['Reference'];
            legend(LegendTitles,'Location','northwest','FontSize',10)
            set(figg1, 'Visible', 'off')
            export_fig(sprintf([ 'H' num2str(stepIGA) '_M' num2str(modalCoeff) '_Pe' num2str(PeConfig(jj)) '_' char(stabMethod) '_CutCL']),'-pdf');

            %% Plot first quarter line of the domain

            figg2 = figure;
            for kk = 1:length(stabDelta)
                M = CutMatrix{:,:,jj,ii,kk};
                plot(linspace(0,Len,N),M(2,:),'-o','LineWidth',1,'MarkerSize',2);
                set(gcf, 'Color', 'w');
                xlim auto
                ylim auto
                grid on
            ...
            xlabel('x [m]');
            ylabel('u(x,y,\delta)');
            plot(linspace(0,Len,N),CutCoordRef(2,:),'--k','LineWidth',1,'MarkerSize',2);
            LegendTitles = cell(1,length(stabDelta));
            for kk = 1:length(stabDelta)
                LegendTitles{kk} = ['\delta = ' num2str(stabDelta(kk))];
            end
            LegendTitles{kk + 1} = ['Reference'];
            legend(LegendTitles,'Location','northwest','FontSize',10)
            set(figg2, 'Visible', 'off')
            export_fig(sprintf([ 'H' num2str(stepIGA) '_M' num2str(modalCoeff) '_Pe' num2str(PeConfig(jj)) '_' char(stabMethod) '_Cut1Q']),'-pdf');

            %% Plot third quarter line of the domain

            figg3 = figure;
            for kk = 1:length(stabDelta)
                M = CutMatrix{:,:,jj,ii,kk};
                plot(linspace(0,Len,N),M(3,:),'-o','LineWidth',1,'MarkerSize',2);
                set(gcf, 'Color', 'w');
                xlim auto
                ylim auto
                grid on
            ...
            xlabel('x [m]');
            ylabel('u(x,y,\delta)');
            plot(linspace(0,Len,N),CutCoordRef(3,:),'--k','LineWidth',1,'MarkerSize',2);
            LegendTitles = cell(1,length(stabDelta));
            for kk = 1:length(stabDelta)
                LegendTitles{kk} = ['\delta = ' num2str(stabDelta(kk))];
            end
            LegendTitles{kk + 1} = ['Reference'];
            legend(LegendTitles,'Location','northwest','FontSize',10)
            set(figg3, 'Visible', 'off')
            export_fig(sprintf([ 'H' num2str(stepIGA) '_M' num2str(modalCoeff) '_Pe' num2str(PeConfig(jj)) '_' char(stabMethod) '_Cut3Q']),'-pdf');

        end
    end
end

%% LOGLOG PLOTS OF THE CONVERGENCE ANALYSIS

for mm = 1:length(stabDelta)
    for jj = 1:length(PeConfig)
        for ii = 1:length(stabMethod)
            ...
            if (selectedPlots(5) == 1)
                figL2 = figure;
                for kk = 1:length(stepIGA)
                    loglog(modalCoeff,ErrorL2Matrix(:,kk,jj,ii,mm),'-o','LineWidth',3,'MarkerSize',3);
                    set(gcf, 'Color', 'w');
                    set(gca, 'FontSize', 14);
                    xlim auto
                    ylim auto
                ...
                loglog(modalCoeff,modalCoeff.^-1,'--','LineWidth',2);
                loglog(modalCoeff,modalCoeff.^-2,'--','LineWidth',2);

                LegendTitles = cell(1,length(stepIGA));
                for kk = 1:length(stepIGA)
                    LegendTitles{kk} = ['h = ' num2str(stepIGA(kk))];
                end
                LegendTitles{kk + 1} = ['Order 1'];
                LegendTitles{kk + 2} = ['Order 2'];
                legend(LegendTitles,'Location','northeast')
                set(figL2, 'Visible', 'off')
                export_fig(sprintf([ char(stabMethod(ii)) '_Pe' num2str(PeConfig(jj)) '_Dlt' num2str(stabDelta(mm)) '_ConvAnalysisL2']),'-pdf');
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%
            % H1-Norm Convergence Plot
            %%%%%%%%%%%%%%%%%%%%%%%%%%

            if (selectedPlots(5) == 1)
                figH1 = figure;
                for kk = 1:length(stepIGA)
                    loglog(modalCoeff,ErrorH1Matrix(:,kk,jj,ii,mm),'-o','LineWidth',3,'MarkerSize',3);
                    set(gcf, 'Color', 'w');
                    set(gca, 'FontSize', 14);
                    xlim auto
                    ylim auto
                ...
                export_fig(sprintf([ char(stabMethod(ii)) '_Pe' num2str(PeConfig(jj)) '_Dlt' num2str(stabDelta(mm)) '_ConvAnalysisH1']),'-pdf');
            end
        end
    end
end