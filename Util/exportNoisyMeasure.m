function [dataFileSurf,errH1,errL2] = exportNoisyMeasure (size_mb,a_ril,b_ril,cutx,hx,...
    ...
    noisyMeasure   = zeros(2*nnx,y_elements);
    noisyMeasure_x = zeros(2*nnx,y_elements);
    noisyMeasure_y = zeros(2*nnx,y_elements);
    ...
    for h = 1:2*nnx
        ...
        for k = 1:M
            ...
            for imb = 1:size_mb
                ...
                noisyMeasure(h,k)   = noisyMeasure(h,k)   + u(h+(imb-1)*2*nnx)*coeffModalBase(k,imb);

                % COMPUTATION OF THE DERIVATIVE OF THE APPROXIMATED
                % SOLUTION VECTOR FIELD EVALUATED IN THE POINTS OF THE
                % DOMAIN MESH

                noisyMeasure_x(h,k) = noisyMeasure_x(h,k) + ux(h+(imb-1)*2*nnx)*coeffModalBase(k,imb);
                noisyMeasure_y(h,k) = noisyMeasure_y(h,k) + u(h+(imb-1)*2*nnx)*coeffModalBaseDer(k,imb);

            end

        noisyMeasure(h,k) = noisyMeasure(h,k)+ a_ril(1) * evalNodesY(k) + b_ril(1);
    ...
    % dataFileSurf = 0;
        ...
        dataFileSurf = sprintf('dataNoisy.%d.vtu',iteration - 1);
        TRI = delaunay(X,Y);
        vtktrisurf(TRI,X,Y,noisyMeasure','ADRConcentration',dataFileSurf);
    ...
    err1 = abs(sol_ex'   - noisyMeasure  );
    err2 = abs(sol_exdx' - noisyMeasure_x);
    err3 = abs(sol_exdy' - noisyMeasure_y);
