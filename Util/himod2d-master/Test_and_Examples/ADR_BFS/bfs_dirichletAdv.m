...
load ./adv/allfields.mat %it loads B1R B1L B2L B2R
mu    = @(x,y) (  1 + 0*x+0*y ); % Diffusione
sigma = @(x,y) (  0 + 0*x+0*y ); % Reazione
chi = 1;
Coeff_formaR = struct('mu',mu,'beta1',B1R,'beta2',B2R, ...
    'sigma',sigma,'coeffrobin',chi);
Coeff_formaL = struct('mu',mu,'beta1',B1L,'beta2',B2L, ...
    'sigma',sigma,'coeffrobin',chi);
...
while( residual > toll )
    display(c)
    %Solve on the left
    [uL,aRilL,bRilL]=solver_DD(Omega1DL,mL,hx, bc_up, bc_downL,dataUp,dataDownL,DatiL,Coeff_formaL,Dati_geometriciL,-1,true,'');
    %compute dirichlet data for the right
    dirOld=DatiR.dato_dir;
    DatiR.dato_dir=BFS_interface(uL,aRilL,bRilL,mL,mR,bc_up{1},bc_downL{1},bc_downR{1},Coeff_formaL);
    %right part
    [uR,aRilR,bRilR]=solver_DD(Omega1DR,mR,hx, bc_up, bc_downR,dataUp,dataDownR,...
        DatiR,Coeff_formaR,Dati_geometriciR,-1,true,'');
    %compute the neumann data for the left
    neuOld=DatiL.dato_neu;
    DatiL.dato_neu=BFS_interfaceNeu(uR,mL,mR,bc_up{1},bc_downL{1},bc_downR{1},Coeff_formaR,hx);
    %residual computation
    residual = norm(neuOld-DatiL.dato_neu,'inf')/norm(neuOld,'inf') + ...
        norm(dirOld-DatiR.dato_dir,'inf')/norm(dirOld,'inf');
    %plot
    plot_solution_DD_BFS(mR,aRilR,bRilR,Omega1DR,hx,uR{1},bc_up{1},bc_downR{1},Coeff_formaL,...
