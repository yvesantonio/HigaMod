...
#archi_x  = open('esattadirdir.out','r')
archi_x1 = open('esattadirrob.out','r')
archi_x2 = open('esattaneuneu.out','r')
archi_f  = open('superconv.out','r')
...
#plt.subplot(8,1,(1,2))
#C=plt.contourf(xx, yy, ua)
##plt.contour(C, levels=C.levels[::1],colors = 'black',hold='on')
#plt.title(r'Dirichlet/Dirichlet, $u_{exact}$ ')
#plt.colorbar(C,use_gridspec=True)
#plt.grid()

plt.subplot(6,1,(1,2))
#plt.subplot(8,1,(3,4))
C1=plt.contourf(xx1, yy1, ua1)
#plt.contour(C1, levels=C1.levels[::1],colors = 'black',hold='on')
plt.title(r'Dirichlet/Robin, $u_{exact}$')
...
#plt.subplot(8,1,(5,6))
plt.subplot(6,1,(3,4))
F=plt.contourf(xx2, yy2, ua2)
#plt.contour(F, levels=C.levels[::1],colors = 'black',hold='on')
plt.title(r'Neumann/Neumann, $u_{exact}$')
plt.colorbar(F,use_gridspec=True)
plt.grid()

plt.subplot(6,1,(5,6))
#plt.subplot(8,1,(7,8))
C2=plt.contourf(xxf, yyf, ufem)
#plt.contour(C2, levels=C.levels[::1],colors = 'black', hold='on')
plt.title(r'Super convergent test case, $u_{exact}$')
plt.colorbar(C2,use_gridspec=True)
plt.grid()

#plt.show
plt.tight_layout()
plt.savefig('Esatte.eps',format='eps',transparent='true')
plt.savefig('Esatte.png',format='png',transparent='true')