function [femb,femb_x]=fembasis(degree,vert, quad_node)

% numero di intervalli
  ne = length(vert)-1;
% Numero di nodi di quadratura in ciascun intervallo
  nqnx = length(quad_node) / ne;
% sono solo due righe, gli zeri non vengono mai memorizzati in ogni riga
% c'è un pezzo della funzione di base
  femb = zeros(length(quad_node),degree+1);
  femb_x = zeros(length(quad_node),degree+1);

  for ie = 1:ne
    xl = vert(ie);             % lato sinistro dell'intervallo
    xr = vert(ie+1);           % lato destro dell'intervallo
    for iqn = 1:nqnx   
      i_x = (ie-1)*nqnx+iqn;
      x = quad_node(i_x);%estraggo i nodi di integrazione di questo intervallo
...
function [femb,femb_x]=fem_bas(n,x,xl,xr)
  ...
  elseif (n == 2) %ef quadratici
    femb =   [ (xr-x)*(x-xm)/(h*(xl-xm)); (x-xl)*(x-xr)/((xm-xl)*(xm-xr)); (x-xl)*(x-xm)/(h*(xr-xm))];
