function [Al,bl]=FSIAssembleVel1DYSecOrd(Map,wyq,mb_k,mb_r,mb_yk,mb_yr,ne,...
...
r00v =(nu*Map.Jac.*(Map.D.^2+2*Map.J.^2) ) *(mb_yk.*mb_yr.*wyq ) ...
     + (    3/2*Map.Jac/dt ) *(mb_k.*mb_r.*wyq ) ...
     + (    Map.Jac.*(Map.D.*Bx+Map.J.*By))*(mb_yk .*mb_r .*wyq) ...
     + E*2/3*dt*Map.Aup*MBrobKY(1)*MBrobRY(1) ...
     + E*2/3*dt*Map.Adown*MBrobKY(2)*MBrobRY(2);
