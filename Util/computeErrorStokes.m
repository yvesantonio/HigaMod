function [errStruct] = computeErrorStokes(plotStruct)
...
%     cd ..
    ...
    cd ..
    cd('Demos/Reference/Stokes')
    
    [p,e,t,ffSolP,ffSolUx,ffSolUy,Ap,Mp,structMesh] = importSolutionStokes('ffMesh.msh','ffSolP.sol','ffSolUx.sol','ffSolUy.sol','Ap.txt','Mp.txt','Au.txt','Mu.txt');
    ...
    aux = ffSolP;
    ffSolP = aux + 0.5 * (max(ProjP) - min(ProjP));
    ...
    errP  = ProjP  - ffSolP;
    errUx = ProjUx - ffSolUx;
    errUy = ProjUy - ffSolUy;
    
    errL2_P  = sqrt(errP  * Mp * errP');
    errL2_Ux = sqrt(errUx * Mp * errUx');
    errL2_Uy = sqrt(errUy * Mp * errUy');
    
    errH1_P  = sqrt(errL2_P^2  + errP  * Ap * errP');
    errH1_Ux = sqrt(errL2_Ux^2 + errUx * Ap * errUx');
    errH1_Uy = sqrt(errL2_Uy^2 + errUy * Ap * errUy');
    
    errStruct.errL2_P  = errL2_P;
    errStruct.errL2_Ux = errL2_Ux;
    errStruct.errL2_Uy = errL2_Uy;
    errStruct.errH1_P  = errH1_P;
    errStruct.errH1_Ux = errH1_Ux;
    errStruct.errH1_Uy = errH1_Uy;
    ...
    fig4 = figure('visible','off');
    pdeplot(p,e,t,'XYData',ffSolP','Contour','on','ColorMap','jet');
    title('Freefem - Pressure P')
    
    fig5 = figure('visible','off');
    pdeplot(p,e,t,'XYData',ffSolUx','Contour','on','ColorMap','jet');
    title('Freefem - Velocity Ux')
    
    fig6 = figure('visible','off');
    pdeplot(p,e,t,'XYData',ffSolUy','Contour','on','ColorMap','jet');
...
%     % EXPORT .VTK FOR HIGAMOD SOLUTION
%     
...
%     xyz           = [p(:,1),p(:,2),zeros(size(p(:,1)))]';
%     uvwHigaMod    = [ProjUx,ProjUy,zeros(size(ProjUx))]';
%     
%     fileVTK = 'HigaMod_Stokes.vtk';
%     fid = fopen(fileVTK,'w');
%     vtk_puvw_write (fid, fileVTK, node_num, element_num, ...
%                     element_order, xyz, element_node', ProjP, uvwHigaMod )
%     
%     % EXPORT .VTK FOR FREEFEM++ SOLUTION
%     
%     element_num   = structMesh.NumbElements;
%     element_order = structMesh.ElementOrder;
%     node_num      = structMesh.NumbNodes;
%     element_node  = structMesh.Elements;
%     xyz           = [p(:,1),p(:,2),zeros(size(p(:,1)))]';
%     uvwFreefem    = [ffSolUx,ffSolUy,zeros(size(ffSolUx))]';
%     
%     fileVTK = 'Freefem_Stokes.vtk';
%     fid = fopen(fileVTK,'w');
%     vtk_puvw_write (fid, fileVTK, node_num, element_num, ...
%                     element_order, xyz, element_node', ffSolP, uvwFreefem )
    
    %%%%%%%%%%%%%%
    % SAVE PLOTS %
    %%%%%%%%%%%%%%
    
    print(fig1 ,'HigaMod_Contour_P','-dpng');
    print(fig2 ,'HigaMod_Contour_Ux','-dpng');
    print(fig3 ,'HigaMod_Contour_Uy','-dpng');
    print(fig4 ,'Freefem_Contour_P','-dpng');
    print(fig5 ,'Freefem_Contour_Ux','-dpng');
    print(fig6 ,'Freefem_Contour_Uy','-dpng');
    print(fig7 ,'Quiver','-dpng');
    print(fig8 ,'Original_HigaMod_P','-dpng');
    print(fig9 ,'Original_HigaMod_Ux','-dpng');
    print(fig10,'Original_HigaMod_Uy','-dpng');
